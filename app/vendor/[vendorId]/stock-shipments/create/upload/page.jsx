    ...
    if (file) {
      ...
      reader.onload = (event) => {
        try {
          ...
          if (Array.isArray(parsedData.shipments) && parsedData.shipments.length > 0) {
            setSelectedFile(parsedData);
          } else {
            alert("Invalid JSON format: 'shipments' array missing or empty.");
          }
        } catch (error) {
        ...
        {uploadResult && (
          <div className="bg-white shadow-md rounded-lg p-6">
            ...
            <p className="text-gray-600 mb-2">
              <strong>Total Shipments:</strong> {selectedFile?.shipments?.length || 0}
            </p>
            <p className="text-gray-600 mb-2">
              <strong>Successfully Uploaded:</strong> {uploadResult.addedCount || 0}
            </p>
            <p className="text-gray-600 mb-2">
              <strong>Invalid Shipments:</strong> {uploadResult.invalidShipments ? uploadResult.invalidShipments.length : 0}
            </p>

            {/* Invalid Shipments Section */}
            {uploadResult.invalidShipments && (
              <div className="mt-4">
                <h3 className="text-lg font-semibold text-red-600">Invalid Shipments:</h3>
                {paginatedResults(uploadResult.invalidShipments).map((error, index) => (
                  <div key={index} className="bg-red-50 border border-red-400 text-red-600 p-3 rounded-lg mt-2">
                    <p>
                      <strong>Shipment ID:</strong> {error.shipmentId}
                    </p>
                    <p>
            ...
            <div className="pagination-controls flex justify-between items-center mt-4">
              ...
              {uploadResult.invalidShipments &&
                uploadResult.invalidShipments.length > currentPage * itemsPerPage && (
                  <button
                    onClick={() => setCurrentPage(currentPage + 1)}
                    className="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600"
                  >
