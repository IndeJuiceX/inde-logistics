    ...
    try {
        ...
        if (unprocessedItems.length > 0) {
            ...
            if (retryResult.success) {
                added.push(...retryResult.added); // Add successfully retried items
                errors.push(...retryResult.errors); // Collect retry errors, if any
            }
        }
...
const retryUnprocessedItems = async (unprocessedItems, operationType, retriesLeft) => {
    ...
    const params = {
        RequestItems: {
            [TABLE_NAME]: unprocessedItems || [],  // Safeguard against undefined unprocessedItems
        },
    };
    ...
    try {
        ...
        stillUnprocessed = response.UnprocessedItems?.[TABLE_NAME] || [];

        const successfullyProcessed = unprocessedItems.length - stillUnprocessed.length;
        added.push(...unprocessedItems.slice(0, successfullyProcessed));

        if (stillUnprocessed.length > 0) {
            // Retry the remaining unprocessed items
        ...
        stillUnprocessed = unprocessedItems; // Consider all items unprocessed in case of failure
    }
